...
<div class="admin-main">
    ...
    <blockquote class="layui-elem-quote">
        <form class="layui-form" action="">
            ...
            <!-- <input type="text" name="teacherName" /> -->
            <div class="layui-input-inline" style="width:150px;">
                <select name="dpm_name" id="dpm_name" >
                    <option></option>
                    <option>植物科技学院</option>
                    <option>动物科技学院</option>
                    <option>机电工程学院</option>
                    <option>车辆工程学院</option>
                    <option>经济贸易学院</option>
                    <option>信息技术学院</option>
                    <option>人文科学学院</option>
                </select>
            </div>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/html" id="tpl">
    ...
    <tr>
        ...
        <td>{{ item.user_sex }}</td>
        <td>{{ item.major_id.major_name }}</td>
        <td>{{ item.major_id.dpm_id.dpm_name }}</td>

        <td>
            <a href="javascript:;" data-name="{{ item.user_id }}" data-opt="edit" class="layui-btn layui-btn-mini">编辑</a>
            <a href="javascript:;" data-name="{{ item.user_id }}" data-opt="reset" class="layui-btn layui-btn-mini">重置密码</a>
            <a href="javascript:;" data-user_id="{{ item.user_id }}" data-opt="del" class="layui-btn layui-btn-danger layui-btn-mini">删除</a>
        </td>
    </tr>
    {{# }); }}
</script>
    ...
    layui.use(['paging', 'form','laytpl'], function() {
        ...
        function search(){
            ...
            $.ajax({
                ...
                type : 'post',
                url:'${sjc}/admin/selectUser', //地址
                data : JSON.stringify(
                    {"user_name":userName,"major_id":{
                            "dpm_id":{
                                "dpm_name":dpm_name
                            }
                        }
                    ...
                    if(json.status == '0'){
                        ...
                        $('#content').children('tr').each(function() {
                            ...
                            $that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
                                ...
                                $.get('insertUser.jsp?user_id=' + $(this).data('name'), null, function (form) {
        ...
        $('#add').on('click', function() {
            if(addBoxIndex !== -1)
                return;
            //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
            $.get('insertUser.jsp', null, function (form) {
                addBoxIndex = layer.open({
                    type: 1,
                    title: '添加用户信息',
                    content: form,
